...
export const CompactPartPreview: React.FC<CompactPartPreviewProps> = ({
  ...
  const svgWidth =
    constrainedAspectRatio >= 1 ? size : size * constrainedAspectRatio
  const svgHeight =
    constrainedAspectRatio >= 1 ? size / constrainedAspectRatio : size
...
const renderLShapeCompactPreview = (part: EnhancedCuttingPart) => {
  ...
  const bottomLeftRadius = Math.min(
    part.lShape.bottomLeftRadius || 0,
    leftWidth,
    rightHeight,
  )
  const topLeftCutoutRadius = Math.min(
    part.lShape.topLeftCutoutRadius || 0,
    leftWidth,
    part.height - rightHeight,
  )
  const innerCutoutRadius = Math.min(
    part.lShape.innerCutoutRadius || 0,
    part.width - leftWidth,
    part.height - rightHeight,
  )
  const rightBottomCutoutRadius = Math.min(
    part.lShape.rightBottomCutoutRadius || 0,
    part.width - leftWidth,
    rightHeight,
  )
  ...
  if (innerCutoutRadius > 0) {
    ...
    path += ` A ${innerCutoutRadius},${innerCutoutRadius} 0 0,0 ${
      leftWidth + innerCutoutRadius
    },${cutoutStartY}`
  } else {
  ...
  if (rightBottomCutoutRadius > 0) {
    ...
    path += ` A ${rightBottomCutoutRadius},${rightBottomCutoutRadius} 0 0,1 ${
      part.width
    },${cutoutStartY + rightBottomCutoutRadius}`
  } else {
  ...
  if (bottomLeftRadius > 0) {
    ...
    path += ` A ${bottomLeftRadius},${bottomLeftRadius} 0 0,1 0,${
      part.height - bottomLeftRadius
    }`
  } else {
...
const renderFramePreview = (part: EnhancedCuttingPart) => {
  ...
  const pieces = [
    // Top piece
    {
      ...
      y: 0,
      width:
        frameType === 'type3' || frameType === 'type4'
          ? part.width - 2 * previewFrameWidth
          : part.width,
    ...
    // Left piece
    {
      ...
      height:
        frameType === 'type1' || frameType === 'type2'
          ? part.height - 2 * previewFrameWidth
          : part.height,
      label: 'left',
    },
    // Right piece
