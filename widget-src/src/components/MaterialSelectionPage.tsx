...
const MaterialSelectionPage: React.FC<MaterialSelectionPageProps> = ({
  ...
  const handleAddMaterial = (material: MaterialSearchResult) => {
    ...
    if (!material.variant?.id) {
      ...
      alert(
        "Chyba: Tento materiál nemá dostupnú variantu pre objednávku. Skúste iný materiál.",
      );
      return;
    }
    ...
    <Container maxWidth={false} sx={{ maxWidth: "1920px", mx: "auto", py: 3 }}>
      ...
      {searchResults.length > 0 && (
        <Paper sx={{ mb: 3, p: 3 }}>
          ...
          {showingAvailableOnly && searchResults.length >= 10 && false && (
      ...
      {selectedMaterials.length > 0 && (
        <Paper sx={{ p: 3, mb: 3 }}>
          ...
          >
            <Button variant="outlined" onClick={() => setSelectedMaterials([])}>
              Zmazať všetko
            </Button>
            <Button
              variant="contained"
