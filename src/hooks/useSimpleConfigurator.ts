...
import {
  createPartWithStatus,
  updatePartWithStatus,
} from '../utils/partConfigurationStatus'
...
export const useSimpleConfigurator = (
  ...
  const cuttingRelevantPartsKey = useMemo(() => {
    ...
    return parts
      .map(
        (p) =>
          `${p.id}:${p.width}:${p.height}:${p.quantity}:${
            p.orientation || 'rotatable'
          }`,
      )
      .join('|')
  }, [parts])
  ...
  const totalArea = useMemo(() => {
    return parts.reduce((sum, part) => {
      return (
        sum + Number(part.width) * Number(part.height) * Number(part.quantity)
      )
    }, 0)
  }, [parts])
