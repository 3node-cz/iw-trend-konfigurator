...
export function createRectangleWithCorners(
  ...
  return {
    ...
    strokeDasharray:
      fillColor === 'none'
        ? SVG_RENDERING.dashArray.dashed
        : SVG_RENDERING.dashArray.solid,
  }
}
    ...
    .map(([corner, modification]) => {
      ...
      const getCornerPosition = (corner: string) => {
        switch (corner) {
          ...
          case 'topRight':
            return { x: padding + width, y: padding }
          case 'bottomRight':
            return { x: padding + width, y: padding + height }
          case 'bottomLeft':
            return { x: padding, y: padding + height }
          default:
            return { x: padding, y: padding }
        }
      }
