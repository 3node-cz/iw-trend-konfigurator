 ...
 */
export function groupPartsByBaseId(placedParts: PlacedPart[]): Record<string, { 
  label: string; 
  color: string; 
  count: number 
}> {
  return placedParts.reduce((groups, placedPart) => {
    ...
    if (!groups[baseId]) {
      groups[baseId] = {
        label: placedPart.part.label || `${placedPart.part.width}×${placedPart.part.height}`,
        color: SHEET_VISUALIZATION.partColors[Object.keys(groups).length % SHEET_VISUALIZATION.partColors.length],
        count: 0
      };
    }
    groups[baseId].count++;
...
export function calculateLayoutStats(sheetLayout: SheetLayout): {
  ...
  const totalPlacedParts = sheetLayout.sheets.reduce((sum, sheet) => sum + sheet.placedParts.length, 0);
  const totalRequestedParts = totalPlacedParts + sheetLayout.unplacedParts.length;
  const placementRatio = totalRequestedParts > 0 ? totalPlacedParts / totalRequestedParts : 0;
 ...
 */
export function getPartDimensions(placedPart: PlacedPart): { width: number; height: number } {
 ...
 */
export function formatAreaInSquareMeters(areaInSquareMillimeters: number): string {
  return (Math.round(areaInSquareMillimeters / 1000000 * 100) / 100).toString();
}
