...
export const hasOverlap = (
  ...
  for (const placed of placedParts) {
    const placedWidth = placed.rotation === 90 ? Number(placed.part.height) : Number(placed.part.width);
    const placedHeight = placed.rotation === 90 ? Number(placed.part.width) : Number(placed.part.height);
    
    // Check if rectangles overlap (with gap)
    if (!(x >= placed.x + placedWidth + gap || 
      ...
      if (!hasOverlap(x, y, partWidth, partHeight, placedParts, gap)) {
        ...
        if (y > 0) {
          for (let testY = y - stepSize; testY >= 0; testY -= stepSize) {
            if (!hasOverlap(x, testY, partWidth, partHeight, placedParts, gap)) {
              canMoveDown = true;
              break;
            }
          }
        ...
        if (x > 0 && !canMoveDown) {
          for (let testX = x - stepSize; testX >= 0; testX -= stepSize) {
            if (!hasOverlap(testX, y, partWidth, partHeight, placedParts, gap)) {
              canMoveLeft = true;
              break;
            }
          }
 ...
 */
export const getPartOrientations = (part: Part): Array<{width: number, height: number, rotation: 0 | 90}> => {
...
export const findBestPlacement = (
  ...
  gap: number,
  stepSize: number = 10
): { x: number; y: number; rotation: 0 | 90; width: number; height: number } | null => {
  const orientations = getPartOrientations(part);
  let bestPosition: { x: number; y: number; rotation: 0 | 90; width: number; height: number } | null = null;

  for (const orientation of orientations) {
    const position = findBestPosition(sheet, orientation.width, orientation.height, gap, stepSize);
...
export const calculateOverallEfficiency = (sheets: Sheet[]): number => {
  ...
  const totalSheetArea = sheets.reduce((sum, sheet) => sum + (sheet.sheetWidth * sheet.sheetHeight), 0);
...
export const optimizeCuttingBLF = (
  ...
  log(`📦 Processing parts: ${sortedParts.map(p => `${p.width}×${p.height}mm`).join(', ')}`);
  ...
  while (remainingParts.length > 0) {
    ...
    while (remainingParts.length > 0 && consecutiveFailures < remainingParts.length) {
      ...
      for (let i = 0; i < remainingParts.length; i++) {
        ...
        if (placement) {
          ...
          log(`✅ Placed ${part.width}×${part.height}mm at (${placement.x}, ${placement.y}) rotation: ${placement.rotation}°`);
    ...
    log(`📋 Sheet ${sheetNumber} completed: ${currentSheet.placedParts.length} parts, ${(currentSheet.efficiency * 100).toFixed(1)}% efficiency`);
    ...
    if (partsPlacedOnThisSheet === 0 && remainingParts.length > 0) {
      ...
      remainingParts.forEach(part => {
        warn(`  - ${part.width}×${part.height}mm (too large for ${sheetWidth}×${sheetHeight}mm sheet)`);
  ...
  log(`📦 Total parts placed: ${sheets.reduce((sum, sheet) => sum + sheet.placedParts.length, 0)}`);
