import type { Part, CornerModification, EdgeTreatment, LShapeConfig } from '../types/simple';
 ...
 */
export const hasAnyCornerModifications = (corners?: Part['corners']): boolean => {
...
export const computePartConfigurationStatus = (part: Part): {
  ...
  const hasAdvancedConfig = hasCornerModifications || hasEdgeTreatments || isLShape;
 ...
 */
export const createPartWithStatus = (partData: Omit<Part, 'id' | 'hasCornerModifications' | 'hasEdgeTreatments' | 'isLShape' | 'hasAdvancedConfig'>): Omit<Part, 'id'> => {
 ...
 */
export const updatePartWithStatus = (part: Part, updates: Partial<Part>): Part => {
