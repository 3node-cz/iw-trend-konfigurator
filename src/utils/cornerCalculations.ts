...
export const calculateMaxCornerRadius = (
  ...
  const getAdjacentCornerRadius = (cornerName: string): number => {
    const adjCorner = corners[cornerName] || { type: 'none' }
    return adjCorner.type === 'round' ? adjCorner.x || 0 : 0
  }

  // For radius, we need to consider both edges that meet at this corner
  // The radius is limited by the smaller of the two available spaces
  ...
  switch (corner) {
    case 'topLeft':
      maxFromWidth -= getAdjacentCornerRadius('topRight') // Same top edge
      maxFromHeight -= getAdjacentCornerRadius('bottomLeft') // Same left edge
      break
    case 'topRight':
      maxFromWidth -= getAdjacentCornerRadius('topLeft') // Same top edge
      maxFromHeight -= getAdjacentCornerRadius('bottomRight') // Same right edge
      break
    case 'bottomRight':
      maxFromWidth -= getAdjacentCornerRadius('bottomLeft') // Same bottom edge
      maxFromHeight -= getAdjacentCornerRadius('topRight') // Same right edge
      break
    case 'bottomLeft':
      maxFromWidth -= getAdjacentCornerRadius('bottomRight') // Same bottom edge
      maxFromHeight -= getAdjacentCornerRadius('topLeft') // Same left edge
      break
  }
    ...
    if (corner1Dim + corner2Dim > maxDimension) {
      ...
      if (updatedCorner === corner1) {
        ...
        if (corner2Data.type === 'round') {
          ...
          result[corner2] = {
            ...corner2Data,
            [dimension]: Math.max(0, remaining),
          }
        }
      }
