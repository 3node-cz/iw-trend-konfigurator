...
const PartRect = styled.rect<{ $index: number }>`
  fill: ${props => {
    return SHEET_VISUALIZATION.partColors[props.$index % SHEET_VISUALIZATION.partColors.length];
  }};
...
export const SheetVisualization: React.FC<SheetVisualizationProps> = ({ sheetLayout }) => {
  ...
  const { scaledWidth, scaledHeight } = calculateSheetScale(sheetWidth, sheetHeight);
  ...
  const { totalPlacedParts, totalRequestedParts } = calculateLayoutStats(sheetLayout);
      ...
      <SheetContainer>
        ...
        >
          ...
          {placedParts.map((placedPart) => {
            ...
            const { width: partWidth, height: partHeight } = getPartDimensions(placedPart);
          ...
          <text 
            ...
            fontSize={SHEET_VISUALIZATION.fontSize.dimensions} 
            fill={SHEET_VISUALIZATION.colors.dimensionText} 
            transform={`rotate(-90, -${SHEET_VISUALIZATION.spacing.dimensionOffset}, ${sheetHeight / 2})`}
          >
            {sheetHeight}mm
          </text>
        </SheetSVG>
        ...
        <InfoGrid>
          ...
          <InfoCard>
            <span className="value">{totalPlacedParts}/{totalRequestedParts}</span>
            <div className="label">Umiestnené dielce</div>
          </InfoCard>
          
          <InfoCard>
            <span className="value">{(sheetLayout.overallEfficiency * 100).toFixed(1)}%</span>
          ...
          <InfoCard>
            <span className="value">{formatAreaInSquareMeters(wastedArea)} m²</span>
        ...
        {Object.keys(partGroups).length > 0 && (
          <Legend>
            {Object.entries(partGroups).map(([id, group]) => (
              <LegendItem key={id} $color={group.color}>
        ...
        {sheetLayout.unplacedParts.length > 0 && (
          <div style={{ marginTop: '16px', padding: '12px', backgroundColor: '#fff3cd', borderRadius: '6px', border: '1px solid #ffeaa7' }}>
            <h4 style={{ color: '#856404', margin: '0 0 8px 0' }}>Neumiestnené dielce:</h4>
            {sheetLayout.unplacedParts.map(part => (
              <div key={part.id} style={{ color: '#856404', fontSize: '0.9rem' }}>
