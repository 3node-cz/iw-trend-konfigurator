...
const TabButton = styled.button<{ $active: boolean; $hasConfig?: boolean }>`
  ...
  transition: all 0.2s;
  border-bottom: ${(props) =>
    props.$active ? '2px solid #3498db' : '2px solid transparent'};
  position: relative;
...
export const TabbedEditor: React.FC<TabbedEditorProps> = ({
  ...
  useEffect(() => {
    if (selectedPart) {
      ...
      const hasLShapeConfig =
        selectedPart.isLShape || selectedPart.lShape?.enabled
      const hasCornerOrEdgeConfig =
        selectedPart.hasCornerModifications || selectedPart.hasEdgeTreatments
      ...
      if (!isLShapeEnabled) {
        onPartUpdate(selectedPart.id, {
          lShape: {
            ...
            bottomRightWidth: Math.min(
              200,
              Math.floor(selectedPart.width * 0.4),
            ),
            bottomRightHeight: Math.min(
              200,
              Math.floor(selectedPart.height * 0.4),
            ),
            innerCornerRadius: 0,
          },
